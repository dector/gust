...
- `-p <port>`: optional app port, or `app:proxy` ports. `?` asks the OS for a free port (for example `-p '?'`, `-p '?:?'`, `-p '8080:?'`). `?` starts at a path-derived port and probes for a free one, so ports stay the same across launches when available. An empty proxy slot (`-p '?:'`) disables the proxy; an empty app slot cannot be proxied because Gust cannot discover the server's default port. Without `-p`, the app chooses its own port. When an app port is set, Gust passes it to the child as `PORT` (overriding any inherited `PORT`); the app must listen on that port. The proxy port is passed as `GUST_PROXY_PORT`. Without `-p`, Gust leaves `PORT` unchanged. There is a small race between choosing ports and the app/proxy binding them.
...
gust ctl comments --filter all        # include resolved (done) threads
gust ctl comments --filter done,review  # only these states
...
gust ctl comments reply <id> <text> --human  # human reply; reopens a review
                                      # thread as submitted for the agent
...
Comments persist across restarts; restarting the app under Gust is not the same
thing. Pi must support blocking subagents for this workflow.
